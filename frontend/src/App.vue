...
const isFileOnlyTool = computed(() => currentTool.value?.inputType === 'file')
    ...
    <main class="relative z-10 pt-24 pb-16 px-6 max-w-7xl mx-auto min-h-screen flex flex-col">
      <transition name="fade" mode="out-in">
        ...
        <div v-else-if="currentView === 'dashboard'" class="py-10">
          ...
          <p class="text-base text-slate-500 dark:text-slate-400 mb-14">{{ t('选择一个专属配置的 Agent 开始您的工作') }}<br/>{{ t('法律声明：本站采用无服务器架构部署，您的数据仅在本地解构分析，不存在泄漏风险。') }}</p><br/>
          <h4 class="text-base text-slate-500 dark:text-slate-400 mb-14">{{ t('fduhfshdiufhisdhifhsidhfihsfih。') }}</h4>
          <p class="text-base text-slate-500 dark:text-slate-400 mb-14">{{ t('法律声明：本站采用无服务器架构部署，您的数据仅在本地解构分析，不存在泄漏风险。') }}</p>
          ...
          <div class="glass-panel p-8 md:p-10 rounded-[2.5rem]">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              <div class="lg:col-span-5 flex flex-col gap-6">
                ...
                <div v-if="!isFileOnlyTool" class="flex p-1.5 bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-white/80 dark:border-slate-700 shadow-inner">
